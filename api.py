...
def alterar(id):
  ...
  try:
    ...
    return jsonify(
        {'message': 'está faltando uma chave no novo valor adicionado'})

  
  if type(valor_id) == int and type(valor_autor) == str and type(
      valor_titulo) == str:
    ...
    return jsonify({
        'message':
        'Os tipos de valores das chaves "id","autor","titulo" tem que ser respectivamente: inteiro, string, string'
    })
...
def adicionar():
  ...
  if type(valor_id) == int and type(valor_autor) == str and type(
    ...
    return jsonify(
        {'message': 'os valores id, autor ou titulo não podem ser nulos'})
  else:
    return jsonify({
        'message':
        'Os tipos de valores das chaves "id","autor","titulo" tem que ser respectivamente: inteiro, string, string'
